...
def main():
    ...
    if choice == 'About':       #About
        ...
        image = Image.open('ogp_img_fc.png')     #画像読み込み
        st.image(image)

        st.markdown("FISH TANK【BUCK-TICK】")      #見出し設定
        st.text("Version 0.1")

        st.markdown("For More Information check out   (https://fc.buck-tick.com/)")     #FISH-TANK URL
